...
```csharp
public async Task<UpdateUserResult> PatchUserAsync(string userId, JsonPatchDocument<UpdateUserDataMo> model, CancellationToken cancellation)
{
    try
    {
        var user = await GetUserByIdAsync(userId, cancellation);
...
این مستند توضیح می‌دهد که تگ‌های `<NoWarn>` و `<NuGetAuditMode>` در
فایل‌های `.csproj` چه کاربردی دارند و چگونه می‌توان هشدارها را در سطح
...
-   از `<NuGetAuditMode>` برای کنترل سطح بررسی امنیتی پکیج‌ها استفاده
    کنید.
-   از `#pragma warning disable/restore` برای خاموش کردن هشدارها در سطح
    فایل یا متد استفاده کنید.
-   بهتر است هشدارها فقط در صورت لزوم خاموش شوند، چون ممکن است خطاهای
    مهم را پنهان کنند.


------------------------------------------------------------------------

# راهنمای اتصال گوشی اندروید با USB و اجرای برنامه MAUI Blazor Hybrid

1) فعال‌سازی گزینه‌های توسعه‌دهنده (Developer Options) در گوشی:
   - مسیر: Settings > About Phone > Build Number
   - 7 بار متوالی روی Build Number ضربه بزنید تا پیام "You are now a developer!" ظاهر شود.

2) فعال‌سازی USB Debugging:
   - مسیر: Settings > Developer Options > USB Debugging
   - گزینه را روشن کنید.

3) نصب درایور USB گوشی در ویندوز (در صورت نیاز):
   - از سایت سازنده گوشی، درایور OEM USB مربوط به مدل خود را دانلود و نصب کنید.
   - اگر در Device Manager ویندوز، گوشی با علامت مثلث زرد دیده شد، درایور مشکل دارد.

4) اتصال گوشی با کابل USB:
   - از کابل با قابلیت انتقال داده استفاده کنید (کابل شارژ معمولی کافی نیست).
   - گوشی را به پورت USB 2.0 (رنگ سیاه/سفید) وصل کنید.
   - پس از اتصال، روی گوشی پیام "Allow USB debugging?" ظاهر می‌شود. گزینه "Always allow from this computer" را انتخاب کرده و Allow بزنید.

5) تنظیم حالت USB روی PTP یا انتقال فایل:
   - از نوار نوتیفیکیشن گوشی، حالت USB را به "Transfer files / MTP" یا "PTP" تغییر دهید.
   - حالت "Charge only" گاهی باعث می‌شود ADB دستگاه را نبیند.

6) بررسی شناسایی گوشی با دستور ADB:
   - دستور adb devices را در ترمینال اجرا کنید.
   - خروجی مطلوب: List of devices attached / XXXXXXXX    device
   - اگر unauthorized دیدید: گوشی را قفل و باز کنید و اجازه دیباگ را بدهید.
   - اگر لیست خالی بود: مشکل از کابل، درایور یا پورت USB است.

7) اجرای برنامه از ویژوال استودیو:
   - پروژه MAUI Blazor Hybrid را در Visual Studio 2022 باز کنید.
   - از نوار ابزار، دستگاه هدف را به گوشی خود تغییر دهید (نام گوشی باید در لیست باشد).
   - کلید Start (F5) را بزنید تا برنامه روی گوشی Build و Deploy شود.

8) عیب‌یابی وقتی ویژوال استودیو گوشی را نمی‌شناسد:

   راه‌حل 1 – کابل و پورت USB را عوض کنید:
      - از کابل اورجینال یا دارای علامت داده استفاده کنید.
      - پورت USB دیگری (ترجیحاً 2.0) امتحان کنید.

   راه‌حل 2 – درایور گوشی را به‌روز کنید:
      - Device Manager را باز کنید.
      - روی دستگاه ناشناس یا گوشی (با مثلث زرد) راست کلیک کنید.
      - Update driver > Browse my computer > Let me pick > Android Device یا MTP USB Device را انتخاب کنید.

   راه‌حل 3 – سرور ADB را ریست کنید:
      - adb kill-server
      - adb start-server
      - adb devices   (بررسی مجدد)

   راه‌حل 4 – کامپوننت USB Device Connectivity ویژوال استودیو را نصب کنید:
      - Visual Studio Installer > Modify > Individual components
      - عبارت "USB Device Connectivity" را جستجو و نصب کنید.

   راه‌حل 5 – گزینه‌های پیش‌نمایش را بررسی کنید:
      - Tools > Options > Environment > Preview Features
      - گزینه ".NET Mono Debugger for MAUI apps" را غیرفعال کنید (در صورت وجود).

   راه‌حل 6 – گوشی و کامپیوتر را ریستارت کنید:
      - گوشی و کامپیوتر را مجدداً راه‌اندازی کنید و مراحل 1 تا 4 را دوباره انجام دهید.

9) مثال عملی دیباگ:
   - پس از اتصال موفق، در VS روی نام گوشی کلیک کنید و F5 بزنید.
   - برنامه روی گوشی اجرا می‌شود.
   - برای دیباگ وب‌ویو (Blazor)، در مرورگر کروم روی کامپیوتر آدرس chrome://inspect را باز کنید، دستگاه را انتخاب کنید و روی inspect کلیک کنید.

10) خلاصه دستورات و تنظیمات پرکاربرد:
    - Developer Options: فعال‌سازی با 7 بار کلیک روی Build Number
    - USB Debugging: روشن در Developer Options
    - adb devices: نمایش دستگاه‌های متصل
    - adb kill-server / start-server: ریست سرور ADB
    - حالت USB MTP یا PTP: تنظیم در نوتیفیکیشن گوشی
    - USB Device Connectivity: کامپوننت مورد نیاز VS
    - کابل داده: حتماً با قابلیت انتقال داده

نکات تکمیلی:
- اگر چندین نمونه VS باز دارید، همه را ببندید و فقط یک نمونه اجرا کنید.
- از پروکسی یا VPN هنگام اتصال استفاده نکنید.
- در لینوکس/مک نیازی به درایور جداگانه نیست، اما ممکن است نیاز به تنظیم udev rules باشد.
- برای اجرا بدون کابل USB می‌توانید از Wi‑Fi ADB استفاده کنید (adb tcpip 5555).

اگر مشکل همچنان باقی است، خروجی adb devices و وضعیت Device Manager را بررسی کنید.



------------------------------------------------------------------------

## لیست Theme های مختف برای vs ادیتور 
